...
def generate(args):
    ...
    def generate_image(prompt, idx, model, n_images=1, image_folder=args.save_image_dir):
        image_paths = []
    ...
    for i, prompt in enumerate(prompts):
        idx = i + args.start
        generated_image_paths.append(generate_image(prompt, idx, model_pipe, args.n_images_per_prompt))
    ...
    # write to csv
    prompt_df.to_csv(os.path.join(args.save_dir, f"prompt_{args.start}.csv"), index=False)
    image_df.to_csv(os.path.join(args.save_dir, f"image_{args.start}.csv"), index=False)
    correlation_df.to_csv(os.path.join(args.save_dir, f"correlation_{args.start}.csv"), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sentence_transformers_weights_path", type=str, default="/home/thanh/shared_disk/thanh/sditp/data/all-MiniLM-L6-v2"
    )
    ...
    # sample usage
    # python generate.py --save_dir ./generated_data --start 0 --end 3000 --n_images_per_prompt 4 --sentence_transformers_weights_path /home/thanh/shared_disk/thanh/sditp/data/all-MiniLM-L6-v2
